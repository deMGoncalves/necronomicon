...
**Intenção:** Mantém uma lista de objetos afetados por uma transação de negócio e coordena a escrita de mudanças e a resolução de problemas de concorrência.
...
- Para evitar múltiplos round-trips ao banco em uma única transação
- Quando precisa rastrear mudanças em objetos de domínio ao longo de uma request
- Em aplicações com Domain Model rico onde persistência é separada do domínio

## Quando NÃO Usar

- Operações simples de CRUD single-entity (overengineering — rule 064)
...
- [identity-map.md](identity-map.md): complementa — Identity Map garante que Unit of Work rastreie instâncias únicas por identidade
- [rule 021 - Proibição de Duplicação de Lógica](../../../rules/021_proibicao-duplicacao-logica.md): reforça — centraliza toda a lógica de persistência em um único ponto de commit
