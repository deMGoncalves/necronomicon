---
name: anti-patterns
description: "Catálogo de 26 anti-patterns com sintomas e refatorações. Use quando @reviewer identificar code smells relacionados às rules 052-070, ou @developer quiser entender o que evitar."
model: haiku
allowed-tools: Read
metadata:
  author: deMGoncalves
...
- **Sugerir refatorações** baseadas em padrões documentados
- **Mapear rules a anti-patterns** (rules 052-070 são anti-patterns)

Triggers específicos:
- @reviewer identifica problema mas não sabe nomeá-lo
- @developer pergunta "o que há de errado com esse código?"
...
| **Primitive Obsession** | 003 | String/number em vez de Value Object | [[primitive-obsession.md]] |

### 🟠 Altos (Exigem Justificativa)

| Pattern | Rule | Sintoma Principal | Arquivo |
|---------|------|-------------------|---------|
| **Long Method** | 055 | Método com > 20 linhas, múltiplas responsabilidades | [[long-method.md]] |
| **Large Class** | 007 | Classe com > 300 linhas, atributos demais | [[large-class.md]] |
| **Divergent Change** | 054 | Uma classe muda por N motivos diferentes | [[divergent-change.md]] |
| **Shotgun Surgery** | 058 | Uma mudança exige N arquivos alterados | [[shotgun-surgery.md]] |
...
| **Overengineering** | 064 | Solução muito complexa para problema simples | [[overengineering.md]] |
...
### 🟡 Médios (Melhorias Esperadas)

| Pattern | Rule | Sintoma Principal | Arquivo |
|---------|------|-------------------|---------|
| **Feature Envy** | 057 | Método usa dados de outra classe mais que da própria | [[feature-envy.md]] |
...
| **Premature Optimization** | 069 | Otimização sem medição de necessidade | [[premature-optimization.md]] |
...
Leia o código buscando sintomas de cada categoria:
- **Estruturais:** Long Method, Large Class, Data Clumps
- **Comportamentais:** Feature Envy, Divergent Change, Shotgun Surgery
- **De Manutenção:** Lava Flow, Boat Anchor, Speculative Generality

### Passo 2 — Nomear o Pattern

Use a tabela acima para nomear o padrão:
```markdown
❌ "Esse código está confuso."
✅ "Este código apresenta **Pyramid of Doom** (Rule 066): 4 níveis de aninhamento.
   Refatoração sugerida: Guard Clauses para linearizar o fluxo."
```

### Passo 3 — Sugerir Refatoração

Cada arquivo de referência contém:
- ❌ Exemplo problemático
- ✅ Exemplo refatorado
- Estratégias de refatoração (Extract Method, Extract Class, etc.)
...
|------|--------------|-----------|
| 003 | Primitive Obsession | String/number em vez de Value Object |
...
| 054 | Divergent Change | Uma classe, N motivos para mudar |
...
- Não nomear qualquer código ruim como "anti-pattern" sem identificar o padrão específico
- Não refatorar anti-pattern sem entender o contexto — pode introduzir outro
- Não usar anti-patterns como desculpa para reescrever código — refatore incrementalmente
- Não focar em anti-patterns de baixa severidade enquanto ignora críticos
...
- Facilita code reviews: "isso é um Middle Man" comunica mais que "código confuso"
- Previne recorrência: uma vez identificado, o time reconhece em código futuro

**Skills relacionadas:**
- [`clean-code`](../clean-code/SKILL.md) — complementa: anti-patterns são violações das práticas Clean Code
- [`codetags`](../codetags/SKILL.md) — depende: violações de anti-patterns são anotadas com codetags pelo @reviewer
