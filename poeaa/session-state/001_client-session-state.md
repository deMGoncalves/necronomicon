...
**Classification**: Session State Pattern

---

## Intent and Purpose

Store session state on the client between requests.
...
Client Session State solves the problem of storing session state on the client between requests.

This pattern is especially useful in enterprise applications where complexity and scale require structured and well-defined solutions. By applying Client Session State, you get a tested and proven approach that has already been validated in countless real projects.
...
Use Client Session State when:

- You need a solution to store session state on the client between requests
...
Interaction:
Client → Client Session State → External System
```
