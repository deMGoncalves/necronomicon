...
Armazenar o estado de sessão em memória no servidor.
...
O Server Session State resolve o problema de armazenar o estado de sessão em memória no servidor.

Este padrão é especialmente útil em aplicações corporativas onde a complexidade e a escala exigem soluções estruturadas e bem definidas. Ao aplicar o Server Session State, você obtém uma abordagem testada e comprovada que já foi validada em inúmeros projetos reais.
...
Use Server Session State quando:

- For necessária uma solução para armazenar o estado de sessão em memória no servidor
...
Interação:
Cliente → Server Session State → Sistema Externo
```
