...
Tipo base para todos os tipos em uma camada.
...
Layer Supertype resolve o problema de definir um tipo base para todos os tipos em uma camada.

Este padrão é especialmente útil em aplicações empresariais onde a complexidade e a escala exigem soluções estruturadas e bem definidas. Ao aplicar o Layer Supertype, você obtém uma abordagem testada e comprovada que já foi validada em incontáveis projetos reais.
...
Use Layer Supertype quando:

- Você precisa de uma solução para definir um tipo base para todos os tipos em uma camada
...
Interação:
Client → Layer Supertype → Sistema Externo
```
