...
Representação em memória de dados tabulares.
...
Record Set resolve o problema de representar dados tabulares em memória.

Este padrão é especialmente útil em aplicações empresariais onde a complexidade e a escala exigem soluções estruturadas e bem definidas. Ao aplicar o Record Set, você obtém uma abordagem testada e comprovada que já foi validada em incontáveis projetos reais.
...
Use Record Set quando:

- Você precisa de uma solução para representar dados tabulares em memória
...
Interação:
Client → Record Set → Sistema Externo
```
