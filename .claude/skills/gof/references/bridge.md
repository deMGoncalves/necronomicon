...
- Mudanças na implementação não devem impactar clientes da abstração
- Ao ter explosão de classes por herança em dois eixos de variação independentes
- Para esconder detalhes de implementação de clientes
...
- [abstract-factory.md](abstract-factory.md): complementa — Abstract Factory pode criar os objetos de implementação para o Bridge
- [strategy.md](strategy.md): complementa — Strategy substitui algoritmo em runtime; Bridge separa hierarquias permanentemente
- [rule 014 - Princípio de Inversão de Dependência](../../../rules/014_principio-inversao-dependencia.md): reforça — abstração depende da interface Renderer, não de implementações concretas
- [rule 011 - Princípio Aberto/Fechado](../../../rules/011_principio-aberto-fechado.md): reforça — adicionar novas formas ou renderers sem modificar código existente
- [rule 064 - Proibição de Overengineering](../../../rules/064_proibicao-overengineering.md): reforça — não usar quando há apenas uma implementação
